...
import { useDuplicateCustomerLink } from "@/lib/api/customers";
...
import { DUPLICATE_LINK_ASPECTS, DUPLICATE_LINK_ASPECT_LABELS, type DuplicateLinkAspect } from "@/types/customer";
...
/** Duplicar vínculo para outra empresa (seção 69 do prompt de clientes) — nunca duplica o
 * cadastro geral, apenas cria um novo vínculo com os itens selecionados. */
export function DuplicateLinkDialog({ open, onOpenChange, linkId, organizationId, currentCompanyId }: DuplicateLinkDialogProps) {
  ...
  const mutation = useDuplicateCustomerLink(linkId);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecione a empresa de destino e os itens que deseja copiar. O cadastro geral do cliente não é
            duplicado — apenas um novo vínculo (prospect) é criado para a empresa selecionada.
          </DialogDescription>
        </DialogHeader>
            ...
            <Alert variant="success">
              ...
              <AlertDescription className="flex flex-col gap-2">
                ...
                <Button asChild variant="outline" size="sm" className="w-fit">
                  <Link href={`/cadastros/clientes/${mutation.data.customerId}/empresas/${mutation.data.id}`}>
                    Ver novo vínculo
                  </Link>
                </Button>
              </AlertDescription>
