...
import { SUPPLIER_ADDRESS_TYPE_LABELS } from "@/types/supplier";
  ...
  return (
    <div className="flex flex-col gap-8">
                ...
                render={({ field }) => (
                  <FormItem className="w-56">
                    ...
                    <Select value={field.value} onValueChange={field.onChange}>
                      ...
                      <SelectContent>
                        {Object.entries(SUPPLIER_ADDRESS_TYPE_LABELS).map(([value, label]) => (
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <FormField
          control={form.control}
          name="emailPaymentReceipts"
          render={({ field }) => (
            <FormItem>
              <FormLabel>E-mail para envio de comprovantes</FormLabel>
              <FormControl>
                <Input type="email" {...field} value={field.value ?? ""} />
              </FormControl>
              <FormMessage />
            ...
            <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div className="flex flex-col gap-2 sm:col-span-3">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox
                      checked={form.watch(`contacts.${index}.isCommercialContact`) ?? false}
                      onCheckedChange={(checked) => form.setValue(`contacts.${index}.isCommercialContact`, Boolean(checked))}
                    />
                    Responsável comercial
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox
                      checked={form.watch(`contacts.${index}.isInvoiceResponsible`) ?? false}
                      onCheckedChange={(checked) => form.setValue(`contacts.${index}.isInvoiceResponsible`, Boolean(checked))}
                    />
                    Responsável por notas fiscais
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox
                      checked={form.watch(`contacts.${index}.isBillingResponsible`) ?? false}
                      onCheckedChange={(checked) => form.setValue(`contacts.${index}.isBillingResponsible`, Boolean(checked))}
                    />
                    Responsável por cobrança
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox
                      checked={form.watch(`contacts.${index}.receivesPaymentReceipts`) ?? false}
                      onCheckedChange={(checked) => form.setValue(`contacts.${index}.receivesPaymentReceipts`, Boolean(checked))}
                    />
